                ...
                $sel->execute();
                  ...
                  if($sel->rowCount() > 0){
                    ...
                    while($f = $sel->fetch()){
                      ...
                      echo'<h6> <i class="fas fa-book"></i> '.strtoupper($f['course']).' - MODULE '.base64_decode($_GET['mn']).' - '.strtoupper(base64_decode($_GET['mod'])).'</h6><br>';
            ...
            <?php
              ...
              ?>
                ...
                <h6 style="padding: 15px; border: 1px solid #aaa;border-radius: 5px;">
                  <a href="class-tc.php?tid=<?=base64_encode($fc['id'])?>&mid=<?=base64_encode($fc['mid'])?>&tnum=<?=base64_encode($fc['tnum'])?>&top=<?=base64_encode($fc['topic'])?>&mn=<?=base64_encode($mn)?>&mod=<?=base64_encode($mod)?>" id="mlink" style="color: #000;">
